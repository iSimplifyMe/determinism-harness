...
for task in extraction structured_json open_generation; do
  mkdir -p "$SP/pilot/door3/$task"
  BODY=$(jq -n --rawfile t "$SP/task_$task.txt" '{"model":"gpt-5.6-sol","input":$t,"max_output_tokens":16000}')
  for i in $(seq 1 20); do
    f="$SP/pilot/door3/$task/$i.json"
    [[ -s "$f" ]] && continue
  ...
  echo "done $task: $(ls "$SP/pilot/door3/$task"/*.json 2>/dev/null | wc -l | tr -d ' ')/20"
done
echo "DOOR3 PILOT COMPLETE"
